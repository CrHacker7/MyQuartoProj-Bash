...
- It adds all changes (`git add .`), commits them with the provided message (`git commit -m "$1"`), and pushes the changes to the `master` branch (`git push origin master`).
...
- **Syntax Requirements**: There must be spaces between the brackets and the expression inside them. For example, `[ "$name" = 'Bob' ]` is valid, whereas `[ "$name"='Bob' ]` is not 
